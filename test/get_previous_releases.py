...
def download_binary(tag, args) -> int:
    if Path(tag).is_dir():
        ...
        shutil.rmtree(tag)
    Path(tag).mkdir()
    release_path = 'cubecoin-{}'.format(tag[1:])
    os = 'linux' # TODO
    tarball = 'cubecoin-{tag}-{platform}.tar.gz'.format(
        tag=tag[1:], platform=args.platform)
    tarballUrl = 'https://download.cubecoin.org/{release_path}/{os}/{tarball}'.format(
    ...
    ret = subprocess.run(['tar', '-zxf', tarball, '-C', tag,
                          '--strip-components=1',
                          'cubecoin-{tag}'.format(tag=tag[1:])]).returncode
    if ret:
        return ret
...
def build_release(tag, args) -> int:
    ...
    with pushd(tag):
        ...
        files = ['cubecoind', 'cubecoin-cli', 'cubecoin-tx']
        for f in files:
