...
The CubeCoin Core daemon can be configured to act as a trusted "border
router", implementing the cubecoin wire protocol and relay, making
...
The ZeroMQ feature in CubeCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building cubecoind:
...
For instance:
    ...
    $ cubecoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/cubecoind.tx.raw \
...
These options can also be provided in cubecoin.conf.
...
From the perspective of cubecoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into cubecoind directly. Furthermore, no information is
...
using. CubeCoind appends an up-counting sequence number to each
